...
test.describe("Teil 5 — Flaky Tests", () => {
  ...
  test.describe("Übung 5.2 — Jeder Test legt genau einen Cocktail in den Warenkorb", () => {
    const uebersicht = (page: Page): Locator =>
      page.getByRole("region", { name: "Warenkorb-Übersicht" });

    test("Übung 5.2a — Mojito hinzufügen ergibt 1 Artikel", async ({ page }) => {
      await page.goto("/");
      await page
        .getByRole("article", { name: "Mojito" })
        .getByRole("button", { name: "In den Warenkorb" })
        .click();
      await expect(uebersicht(page)).toContainText("1 Artikel");
    });

    test("Übung 5.2b — Cuba Libre hinzufügen ergibt 1 Artikel", async ({ page }) => {
      await page.goto("/");
      await page
    ...
    test("Übung 5.2c — Ipanema hinzufügen ergibt 1 Artikel", async ({ page }) => {
      await page.goto("/");
      await page
  ...
  test("Übung 5.4 — Mojito zeigt Happy-Hour-Preis (real-time abhängig)", async ({
    ...
    await page.clock.setFixedTime(new Date("2026-07-10 17:30+0200"))
    await page.goto("/");
